    ...
    private fun evaluate(expr: Expr): Value = when (expr) {
        ...
        is Identifier -> { // --> "x" por ejemplo
            val variable = environment.lookup(expr.name)
                ?: throw error(expr, "recurses.Variable '${expr.name}' is not declared")
            variable.value ?: throw error(expr, "recurses.Variable '${expr.name}' is not initialized")
